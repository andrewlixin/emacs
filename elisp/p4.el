...
;;
;; $Id: p4.el,v 1.5 2016/10/13 01:33:56 xinli Exp $
	  ...
	  (t
	   (let ((link-client-name (get-char-property pnt 'link-client-name))
		...
		link-client-name link-depot-name))
		 ...
		 (if (string-match "^.*#[0-9]+ - .+ change \\([0-9]+\\) .+$" line)
		     (if (find-buffer-by-name (format "Edit change %s" (match-string 1 line)))
			 (switch-to-buffer (format "Edit change %s" (match-string 1 line)))
		       (p4-async-process-command "change" "Description:\n\t"
						 (format "Edit change %s" (match-string 1 line))
						 nil (list (match-string 1 line))))
